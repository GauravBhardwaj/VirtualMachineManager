...
public class RecoveryManager extends Thread {
	...
	{   
		...
		VirtualMachine vm = (VirtualMachine) new InventoryNavigator(rootFolder).searchManagedEntity("VirtualMachine", "TEAM14_Ubuntu10");
	...
	public void run(){
		...
		if(CheckHostState(hostIp).equalsIgnoreCase("alive")){//case3. check if the host machine is dead (vHost )-null should be ip
			...
			try {
				if(getVMState()=="poweredOff" | getVMState()=="suspended"){//since vHost is alive check if user has suspended or stopped it, so restart it. 	
					...
					Snapshot snapVM = new Snapshot("TEAM14_Ubuntu10",null);
		...
		else{ //case4. vHost is dead so try to revert this vHost from its most recent snapshot
			...
			try {
				...
				HostSnapshot snapHost = new HostSnapshot("T14-vHost04-lab2 _.133.25",null);
	...
	//	if(CheckHostState(hostIp).equalsIgnoreCase("alive")){	// checking again if host is back if not then added 
			if(ManageHost.addHost("130.65.133.23")){//case5. vHost is added successfully, migrate machine on this vHost 
				 try {
					ClonedBackUp.migrateClone("TEAM14_Ubuntu09", "130.65.133.23");
